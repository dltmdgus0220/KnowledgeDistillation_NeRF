...
def init_nerf_model(D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False): # 8, 256, 63, 27, 4, [4], True 가 입력됨, output_ch = 4인 이유는 density 1 + rgb 3
    ...
    # print(f"FLOPS: {flops / 10 ** 9:.04} G", f"({flops})")
    return model


# Ray helpers

def get_rays(H, W, focal, c2w): # rays를 tensorflow로 
...
def get_rays_np(H, W, focal, c2w): # rays를 numpy array로
    """Get ray origins, directions from a pinhole camera."""
...
def ndc_rays(H, W, focal, near, rays_o, rays_d):
    ...
    Returns:
      rays_o: array of shape [batch_size, 3]. Camera origin in NDC. origin은 nomalized plane이기 때문에 z축 값이 -1이다.
